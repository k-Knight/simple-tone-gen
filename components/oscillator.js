            ...
            <div class="space-y-3 bg-zinc-900/30 p-4 border border-zinc-800/60 rounded-2xl relative ${gen.isMuted ? 'opacity-40 grayscale-[30%] transition-opacity' : ''}">
                <section class="p-6 bg-zinc-900/80 border border-zinc-800 rounded-xl shadow-lg">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ${knob(gen, 'frequency', 'Frequency', 20, 20000, 1, true, 'Hz', () => appStateInstance.sync(gen.id), appStateInstance)}
                        ${knob(gen, 'loudness', 'Loudness', 0, 1, 0.01, false, '%', () => appStateInstance.sync(gen.id), appStateInstance)}
                        ${knob(gen, 'pan', 'Balance', -1, 1, 0.01, false, 'Bal', () => appStateInstance.sync(gen.id), appStateInstance)}
                        ${knob(gen, 'timeShift', 'Time Shift', 0, 0.05, 0.001, false, 'Sec', () => appStateInstance.sync(gen.id), appStateInstance)}
                    </div>

                    <div class="mt-4 pt-4 border-t border-zinc-800/50">
                        ${gen.effects.length === 0 ? html`
                            <button onClick=${() => appStateInstance.addEffect(gen.id)} class="px-3 py-1.5 border border-purple-900 bg-purple-950/30 hover:bg-purple-900/40 text-purple-400 text-xs font-bold rounded-xl transition-all uppercase tracking-wide cursor-pointer flex items-center gap-2">
                                <div class="flex items-center justify-center">${icons.add.cloneNode(true)}</div>
                                <span>Add Super Effect</span>
                            </button>
                        ` : gen.effects.map(fx => html`
                            <div class="p-4 bg-zinc-950/40 border border-purple-900/30 rounded-xl space-y-3">
                                <div class="flex items-center justify-between">
                                    ...
                                    <button onClick=${() => appStateInstance.removeEffect(gen.id, fx.id)} class="text-zinc-400 hover:text-rose-400 cursor-pointer p-1 flex items-center justify-center hover:bg-zinc-800 rounded-md transition-colors w-7 h-7" title="Remove Effect">
                                        ${icons.remove.cloneNode(true)}
                                    </button>
                                </div>
